...
int setup_ssl_hooks(void) {
    ...
    if (!libssl_handle) {
        ...
        // This is not a fatal error - we can still capture packets, just not decrypt them
        printf("Continuing without SSL key extraction (encrypted traffic will not be decrypted)\n");
        return 0; // Return success but with no hooks
    }
    ...
    printf("SSL hooks ready for key extraction\n");
    printf("When the target process performs SSL operations, keys will be captured\n");
        ...
        if (getpeername(fd, (struct sockaddr*)&addr, &addr_len) == 0) {
            ...
            if (key_map_fd >= 0) {
                ...
                if (ret == 0) {
                    ...
                    printf("💡 This flow can now be decrypted in real-time\n");
                } else {
